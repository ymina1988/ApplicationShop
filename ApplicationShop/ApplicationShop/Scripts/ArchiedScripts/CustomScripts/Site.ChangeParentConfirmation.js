(function () {
	...
	$(function () {
		...
		$(".requestDetail").on("click", function (e) {
			...
			$.get("/User/UserProfile/GetRelBeforeTreeConf", { txtCodeMelli: codeMBefore },
		...
		"json"
			...
			$.get("/User/UserProfile/GetRelAfterTreeConf", { txtCodeMelli: codeMAtfer },
		function (data, textStatus, jqXHR) {
			var nodeCounter = data.tree.length - 1;
			data.tree.reduce(function (r, a) {
				var o = { id: a.CodeMelli, FullName: a.FirstName + ' ' + a.LastName, Semat: a.Semat_Desc, state: { expanded: true }, icon: "glyphicon glyphicon-user", text: ' ' + a.FirstName + ' ' + a.LastName + ' - ' + a.CodeMelli + ' - ' + a.Semat_Desc, nodes: [] };
				...
				$.ajax({
					...
					success: function (data) {
						...
						if (data.Status === "Success") {
							...
							if (ans === "parentdisagree") {
								item.addClass("disabled");
								item.prev().removeClass("disabled");
							}
							if (ans === "grandparentagree") {
								item.addClass("disabled");
								item.next().removeClass("disabled");
							}
							if (ans === "grandparentdisagree") {
								item.addClass("disabled");
								item.prev().removeClass("disabled");
							}
							if (ans === "destinationagree") {
							    item.addClass("disabled");
							    item.next().removeClass("disabled");
							}
							if (ans === "destinationdisagree") {
							    item.addClass("disabled");
							    item.prev().removeClass("disabled");
							}
							...
							new PNotify({
								title: 'عملیات موفق',
								text: 'جواب شما به درخواست زیر دستی با موفقیت ذخیره شد.',
								type: 'success',
							...
							if (data.Status === "LateAnswer") {
								new PNotify({
									title: 'عملیات ناموفق',
									text: 'شما فقط تا قبل از جواب دادن مدیر برای اعلام نظر خود فرصت دارید.',
									type: 'danger',
									icon: 'glyphicon glyphicon-ok',
									delay: 2000
								});

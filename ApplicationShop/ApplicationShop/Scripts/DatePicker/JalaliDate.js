function JalaliDate() {
    ...
    this.jalaliToGregorian = function (j_y, j_m, j_d) {
        j_y = parseInt(j_y); j_m = parseInt(j_m); j_d = parseInt(j_d); var jy = j_y - 979; var jm = j_m - 1; var jd = j_d - 1; var j_day_no = 365 * jy + parseInt(jy / 33) * 8 + parseInt((jy % 33 + 3) / 4); for (var i = 0; i < jm; ++i) j_day_no += this.j_days_in_month[i]; j_day_no += jd; var g_day_no = j_day_no + 79; var gy = 1600 + 400 * parseInt(g_day_no / 146097); g_day_no = g_day_no % 146097; var leap = true; if (g_day_no >= 36525) {
        ...
        for (var i = 0; g_day_no >= this.g_days_in_month[i] + (i == 1 && leap) ; i++)
            g_day_no -= this.g_days_in_month[i] + (i == 1 && leap); var gm = i + 1; var gd = g_day_no + 1; return [gy, gm, gd];
    }
    ...
    this.gregorianToJalali = function (g_y, g_m, g_d) {
        g_y = parseInt(g_y); g_m = parseInt(g_m); g_d = parseInt(g_d); var gy = g_y - 1600; var gm = g_m - 1; var gd = g_d - 1; var g_day_no = 365 * gy + parseInt((gy + 3) / 4) - parseInt((gy + 99) / 100) + parseInt((gy + 399) / 400); for (var i = 0; i < gm; ++i)
            g_day_no += this.g_days_in_month[i]; if (gm > 1 && ((gy % 4 == 0 && gy % 100 != 0) || (gy % 400 == 0)))
    ...
    this.setDate = function (day) { var diff = -1 * (this.jalalidate[2] - day); var g = this.gregoriandate.setDate(this.gregoriandate.getDate() + diff); this.gregoriandate.setHours(1, 0, 0, 0); this.setJalali(); return g; }
    this.setFullYear = function (year, month, day) {
        var y = parseInt(year); var m = parseInt(month); var d = parseInt(day); if (isNaN(month))
            m = 0; if (isNaN(day))
                d = 1; if (m < 0)
                    m = 0; if (m == 12) { y++; m = 0; }
        if (d < 1)
            d = 1; var j = this.jalaliToGregorian(y, m + 1, d); var retval = this.gregoriandate.setFullYear(j[0], j[1] - 1, j[2]); this.gregoriandate.setHours(1, 0, 0, 0); if (month < 0 || day < 1) {
                if (month < 0) { retval = this.gregoriandate.setMonth(this.gregoriandate.getMonth() + month); }
                if (day < 1) { retval = this.gregoriandate.setDate(this.gregoriandate.getDate() + day - 1); }
                this.setHours(1, 0, 0, 0); this.setJalali();
            }
            else
                this.jalalidate = [y, m, d]; return retval;
    }
    this.setHours = function (hour, min, sec, millisec) { var retval = this.gregoriandate.setHours(hour, min, sec, millisec); this.setJalali(); return retval; }
    ...
    this.setMonth = function (month, day) {
        var y = this.jalalidate[0]; var m = parseInt(month); var d = this.jalalidate[2]; if (isNaN(day) == false)
            d = parseInt(day); return this.setFullYear(y, m, d);
    }
    this.setSeconds = function (s, m) { var retval = this.gregoriandate.setSeconds(s, m); this.setJalali(); return retval; }
    this.setTime = function (m) { var retval = this.gregoriandate.setTime(m); this.gregoriandate.setHours(1, 0, 0, 0); this.setJalali(); return retval; }
    ...
    this.gregoriandate = new Date(); this.gregoriandate.setHours(1, 0, 0, 0); if (arguments.length == 0) { }
    else if (arguments.length == 3) {
        if (arguments[0] == 1900 || arguments[0] == 2099)
            this.gregoriandate.setFullYear(arguments[0], arguments[1], arguments[2]); else
            this.setFullYear(arguments[0], arguments[1], arguments[2]);
    }
    else if (arguments.length == 7) { this.setFullYear(arguments[0], arguments[1], arguments[2]); this.setHours(arguments[3], arguments[4], arguments[5], arguments[6]); }
    else if (arguments.length == 1 && typeof (arguments[0]) === "number") { this.gregoriandate.setTime(arguments[0]); }
    else if (arguments.length == 1 && typeof (arguments[0]) === "JalaliDate") { this.gregoriandate = arguments[0].gregoriandate; }
    else { debugger; }
    this.setJalali();
}
JalaliDate.parse = function (datestring) {
    try {
        ...
        { var date = new Date(parseInt(datestring.replace(/^\/Date\((.*?)\)\/$/, "$1"), 10)); var y = new JalaliDate(date).getFullYear(), m = new JalaliDate(date).getMonth(), d = new JalaliDate(date).getDate(); return new JalaliDate(y, m, d); }
        else { var y = parseInt(datestring.substring(0, 4)), m = parseInt(datestring.substring(5, 7)), d = parseInt(datestring.substring(8, 10)); return new JalaliDate(y, m - 1, d); }
    } catch (e) { return new JalaliDate(1300, 1, 1) }
}